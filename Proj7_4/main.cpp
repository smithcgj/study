...
BTNode * CreateBT1(char *pre,char *in,int n)
{
    ...
    int k;
    if(n<=0) return NULL;
    s=(BTNode *)malloc(sizeof(BTNode));//创建二叉树节点*a
    s->data=*pre;
    for(p=in;p<in+n;p++)//中序序列中找等于*ppos的位置
        if(*p==*pre)
        break;
    k=p-in;
...
BTNode *CreateBT2(char* post, char* in, int n,int m)
{
    ...
    for(p=in;p<in+n;p++)//求in中全部字符中在post中最右边的那个字符
        ...
        //在in中用maxp指向这个字符,用maxin标识它在in中的下标
        if(*p==*q)
        {
            k=q-post;
            if(k>maxpost)
        ...
        s=(BTNode *)malloc(sizeof(BTNode));//创建二叉树节点
        s->data=post[maxpost];
        s->lchild=CreateBT2(post,in,maxin,m);
        s->rchild=CreateBT2(post,maxp+1,n-maxin-1,m);
        return s;
}
void DispBTNode1(BTNode* b)//以凹入表示法输出一棵二叉树
{
    BTNode *St[MaxSize],*p;
